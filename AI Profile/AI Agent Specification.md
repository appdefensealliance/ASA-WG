...
* Verify per-action user consent is requested prior to each invocation of a **Sensitive Action** — an action the Agent classifies as sensitive or non-reversible on its own criteria (irreversible; transfers value or money; mutates or shares user data beyond the current task; or grants or expands access), **independent of tool metadata**. A one-time, first-use consent is not sufficient. The AI Tool's \`consent\_required\` signal is a non-blocking **SHOULD** that the Agent MUST honor **when present**, but its absence MUST NOT prevent the Agent from gating an action it independently classifies as sensitive.  
