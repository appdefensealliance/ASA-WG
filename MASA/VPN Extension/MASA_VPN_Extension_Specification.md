...
## 4. Section 4: Baseline MASA Improvements (General Hardening)

### 4.1 Native-Level Cryptographic Key Auditing
* **Requirement Identifier:** `MASVS-CRYPTO-2` *(Integration: MASA 1.2.2.x)*
* **Proposed Control:** The application must not hardcode static cryptographic keys, passwords, or salts within compiled native binaries, assets, or configuration files, nor derive keys from stable platform/hardware identifiers (`ANDROID_ID`, `Build.SERIAL`, `MAC`, or `IMEI`).
* **Acceptance Criteria:**
  * **PASS:** Cryptographic keys are generated dynamically and stored securely within system Keystore/Keychain.
  * **FAIL:** Statically compiled keys exist in native binaries, or a key is derived from a stable hardware/platform identifier.

---

### 4.2 Software Composition Analysis (SCA) for Native Compiled Libraries
* **Requirement Identifier:** `MASVS-CODE-3` *(Integration: MASA 1.6.2.1)*
* **Proposed Control:** The application must not bundle third-party compiled native libraries (`.so` / `.dylib`) containing unpatched, public vulnerabilities with a severity rating of **CVSS $\ge$ 7.0**.
* **Acceptance Criteria:**
  * **PASS:** All bundled native libraries are fingerprint-resolved and free of unpatched CVSS $\ge$ 7.0 vulnerabilities.
  * **FAIL:** Bundled native libraries contain known vulnerabilities with a CVSS score of 7.0 or greater.

---

### 4.3 Intercepting Plaintext Dynamic Configuration Fetches
* **Requirement Identifier:** `MASVS-NETWORK-1` *(Integration: MASA 1.4.1.x)*
* **Proposed Control:** The application must enforce secure network communication (HTTPS/TLS) with strict hostname verification for all dynamic configuration fetches, server directory retrievals, and runtime API calls. Cleartext (HTTP) configuration fetches are prohibited. *(Remote server TLS scanning is excluded).*
* **Acceptance Criteria:**
  * **PASS:** 100% of dynamic configuration fetches use TLS.
  * **FAIL:** Dynamic configuration fetches or server directory requests are transmitted over cleartext HTTP.

---

### 4.4 Context-Aware Least-Privilege Verification
* **Requirement Identifier:** `MASVS-PRIVACY-1` *(Integration: MASA 1.8.1.1)*
* **Proposed Control:** The application must declare and request only platform permission scopes strictly necessary for core functions. High-risk permissions (`READ_LOGS`, `LOCATION_ALWAYS`, `USE_LOCAL_NETWORK`) are prohibited without explicit technical justification. *(App crashes upon manual permission denial are UX issues and do NOT trigger security failures).*
* **Acceptance Criteria:**
  * **PASS:** Permission manifest contains only justified, core-routing scopes with zero unapproved high-risk permissions.
  * **FAIL:** High-risk permission scopes are declared without valid justification or silently inherited from third-party SDKs.

---

## 5. Section 5: Technical Exclusions & Out-of-Scope Summary

| Domain Area | Exclusion Rationale | Governing Authority |
| :--- | :--- | :--- |
| **Active Censorship Evasion** | Highly volatile, regional, cat-and-mouse transport metric; non-deterministic in lab environments. | Out of Scope |
| **Corporate Ownership / Shell Inspection** | Administrative corporate forensic investigation; unmasking beneficial ownership requires legal/business filings. | Storefront Developer Org Verification (Gate A) |
| **Remote Server Infrastructure Hardening** | Backend cloud endpoints are out of scope; MASA evaluates client binary code safety. | Out of Scope / Internal Ops |
| **Server-Side TLS Scanning** | Avoids out-of-scope remote scanning of third-party server endpoints during client audits. | Excluded from Client Audit |
